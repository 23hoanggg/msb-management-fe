...
export default function CustomerOrderPage() {
  ...
  const { states, computed, setters, actions } = useCustomerOrder(sessionId);

  // --- UI: MÀN HÌNH KHÓA (KẾT THÚC PHIÊN) ---
  if (states.isSessionClosed) {
    return (
  ...
  // --- UI: MÀN HÌNH CHÍNH CỦA KHÁCH HÀNG ---
  return (
    <div className="min-h-screen bg-slate-50 pb-24 md:max-w-md md:mx-auto md:shadow-2xl relative font-sans">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-5 rounded-b-3xl shadow-lg sticky top-0 z-20">
        <div className="flex justify-between items-center mb-2">
          ...
          <Button
            ...
            onClick={() => setters.setIsCartOpen(true)}
          >
            <History className="w-5 h-5 text-white" />
            {states.orderedItems.length > 0 && (
      ...
      <div className="sticky top-[88px] z-10 bg-slate-50/90 backdrop-blur-md py-3 px-4 overflow-x-auto whitespace-nowrap hide-scrollbar border-b border-slate-200/50">
        <div className="flex gap-2">
          <button
            className={`px-5 py-2 rounded-full text-sm font-bold transition-all duration-300 ${states.activeCategory === "ALL" ? "bg-purple-600 text-white shadow-md scale-105" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-100"}`}
            onClick={() => setters.setActiveCategory("ALL")}
          >
            Tất cả
          </button>
          {states.categories.map((cat) => (
            <button
              key={cat.id}
              className={`px-5 py-2 rounded-full text-sm font-bold transition-all duration-300 ${states.activeCategory === cat.id ? "bg-purple-600 text-white shadow-md scale-105" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-100"}`}
              onClick={() => setters.setActiveCategory(cat.id)}
            >
              {cat.name}
            </button>
          ))}
      ...
      <div className="p-4 grid grid-cols-2 gap-4">
        {computed.filteredProducts.map((product) => {
          const qtyInCart = states.localCart[product.id] || 0;
          ...
          return (
            ...
            >
              <div className="h-32 bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-6xl relative overflow-hidden">
                {product.imageUrl ? (
                  <img
                    src={
                ...
                {qtyInCart > 0 ? (
                  <div className="flex items-center justify-between bg-purple-50 rounded-full p-1 border border-purple-100">
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-7 w-7 rounded-full bg-white text-purple-600 shadow-sm"
                      onClick={() => actions.handleUpdateCart(product, -1)}
                    >
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-7 w-7 rounded-full bg-purple-600 text-white shadow-sm"
                      onClick={() => actions.handleUpdateCart(product, 1)}
                    >
                  ...
                  <Button
                    className="w-full h-9 rounded-full text-xs font-bold bg-slate-900 hover:bg-purple-600 text-white"
                    onClick={() => actions.handleUpdateCart(product, 1)}
      ...
      {computed.totalCartItems > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white via-white to-transparent z-20 md:max-w-md md:mx-auto animate-in slide-in-from-bottom-10 pt-10">
          <Button
            className="w-full h-14 bg-purple-600 hover:bg-purple-700 text-white rounded-2xl shadow-xl flex justify-between px-6 text-lg"
            onClick={() => setters.setIsCartOpen(true)}
          >
      ...
      {/* MODAL BOTTOM SHEET (CHI TIẾT GIỎ HÀNG & LỊCH SỬ) */}
      <Dialog open={states.isCartOpen} onOpenChange={setters.setIsCartOpen}>
        <DialogContent className="fixed top-auto bottom-0 left-[50%] translate-x-[-50%] translate-y-0 sm:max-w-md w-full h-[85vh] rounded-t-3xl rounded-b-none p-0 flex flex-col border-none shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-slate-50 custom-scrollbar">
            ...
            {computed.totalCartItems > 0 && (
              <div className="mb-8">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden p-2">
                  {Object.entries(states.localCart).map(
                    ([id, qty], index, arr) => {
                      const p = states.products.find((x) => x.id === id);
                      if (!p) return null;
                      return (
                        <div
                          key={id}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 bg-slate-50 rounded-full p-1 border border-slate-100">
                            <Button
                              ...
                              onClick={() => actions.handleUpdateCart(p, -1)}
                            >
                              <Minus className="w-3 h-3" />
                            </Button>
                            ...
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-7 w-7 rounded-full text-purple-600 bg-purple-100"
                              onClick={() => actions.handleUpdateCart(p, 1)}
                            >
                              <Plus className="w-3 h-3" />
                            </Button>
            ...
            {/* LỊCH SỬ ĐÃ GỬI CHO LỄ TÂN */}
            <div>
              <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                <History className="w-4 h-4" /> Lịch sử đã gửi
              </h4>
              {states.orderedItems.length === 0 ? (
                <div className="text-center py-10 bg-white rounded-2xl border border-slate-100 flex flex-col items-center justify-center">
          ...
          {computed.totalCartItems > 0 && (
            <div className="p-4 bg-white border-t border-slate-100 shrink-0 pb-safe">
              <Button
                className="w-full h-14 text-lg font-black tracking-wide bg-slate-900 text-white rounded-2xl"
              ...
              >
                ...
                XÁC NHẬN GỬI ({computed.totalCartPrice.toLocaleString()}đ)
              </Button>
            </div>
          )}
        </DialogContent>
